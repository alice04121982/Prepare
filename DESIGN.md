...
### Charts
Data charts (the Our World in Data small multiples on /worried, `SaferWorld.tsx`) follow the same frame: a lowercase heavy title, the old figure struck through beside the new one in quantity type, a 10px rule, then a 3px ink line over a Hush area, or solid ink bars, on a 3px ink baseline. Ink only, because these are not kit categories. Axis years and notes are HTML, so they keep their size at any width. Every chart has a "Show the figures" table.
